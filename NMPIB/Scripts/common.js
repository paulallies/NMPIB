...
function getCalendarDate(myDate) {
    ...
    months[10] = "Nov";
    months[11] = "Dec";
    var tempDate = new Date(myDate.match(/\d+/)[0] * 1);
    var monthnumber = tempDate.getMonth();
...
function FillSelect(controlname, itemcollection, url) {
    var options = "";
    $.getJSON(url,
    function(data) {
    $.each(data[itemcollection],
        function(i, item) {
            options += '<option value="' + item.id + '">' + item.name + '</option>';
        });
        $('select[id=' + controlname + ']').html(options);
    });
}
    ...
    if ($(control).val() != "") 
    {
           if (isValidDate($(control).val(), false) == false) {
               if ($('span[id=' + validatorname + ']').length == 0) {
                   $(control).after("<span class='error' style='color:red' id=" + validatorname + ">Invalid Date</span>");
               }
               return false;
        }
        else {
...
function verifyEmpty(control, validatorname){
    if(Trim($(control).val()) == "")
    {
      $(control).after("<span class='error' style='color:red' id=" + validatorname + ">Empty Field</span>");
      return false;
    }
    else
    return true;
...
function verifySelect(control, selectval, validatorname){
    if($(control).val() == selectval)
    {
      $(control).after("<span class='error' style='color:red' id=" + validatorname + ">Please Select</span>");
      return false;
    }
    else
    return true;
...
function verifyNumber(control, validatorname) {
    if ($(control).val() != "") {
        if (isValidNumber($(control).val()) == false) {
            if ($('span[id=' + validatorname + ']').length == 0) {
                $(control).after("<span class='error' style='color:red' id=" + validatorname + ">Invalid Number</span>");
            }
            return false;
        }
        else {
